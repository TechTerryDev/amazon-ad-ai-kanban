...
- 从现在开始统一使用 `vMAJOR.MINOR.PATCH`（例如 `v0.3.0`），不再使用 `release-YYYYMMDD`。
- 版本号含义：
  - `MAJOR`：不兼容变更或重大架构升级；
  - `MINOR`：向后兼容的新功能（你说的“有明显功能新增”通常加这里）；
  - `PATCH`：向后兼容的问题修复/小优化。
- 当前阶段采用 `0.x.y`（产品仍在快速迭代）；当能力和接口趋于稳定后再进入 `1.0.0`。
- 版本变更明细统一记录在 `CHANGELOG.md`。

### 已完成迁移（旧标签 → 新标签）

- `release-20260126` → `v0.1.0`
- `release-20260204` → `v0.2.0`
- `release-20260205` → `v0.2.1`
- `release-20260214` → `v0.3.0`
